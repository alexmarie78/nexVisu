...
def correct_and_unfold_data(flat_image: numpy.ndarray, images: numpy.ndarray, path: str, contextual_data: dict) \
    ...
    for image_index, image in enumerate(images):
        ...
        i1new = i1 / factor_intensity_double_pixel
        i5 = this_corrected_image[line_index5, :]
        i5new = i5 / factor_intensity_double_pixel
        i3 = (i1new + i5new) / 2.0
        ...
        unfolded_data.append((image_index, tth, psi, corrected))
...
def get_angles(path: str) -> (numpy.ndarray, numpy.ndarray):
    with File(path, mode='r') as h5file:
        ...
        if delta_array is None:
            for delta_path in MetadataPath.Delta:
                try:
                    if delta_array is None:
                        delta_array = get_dataset(h5file, delta_path.value)[:]
                except AttributeError:
        ...
        if gamma_array is None:
            for gamma_path in MetadataPath.Gamma:
                try:
                    if gamma_array is None:
                        gamma_array = get_dataset(h5file, gamma_path.value)[:]
                except AttributeError:
